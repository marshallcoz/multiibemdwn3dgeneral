...
cmd_fig_1=['cd(para.nomcarpeta);pause(.1);if(ishandle(para.bar));',...
...
bouton.dim=uicontrol('parent',uipm,'Style','popupmenu',...
    ...
    'position',[0.01 0.75 .15 .2],'string',{'2D','2.5D','3D axisim','3D arbit'},...
...
bouton.gfThisPieceKind = uicontrol('parent',bouton.geoFilePanel,'Style','popupmenu','BackgroundColor',c_3,'Units','normalized',...
    'position',[0.0 0.25 0.4 0.25],'string',{'Free surface','Continuity with the medium #:','Just to close the polyhedron'},...  
...
bouton.gfThisPieceContinuosTo = uicontrol('parent',bouton.geoFilePanel,'Style','edit','BackgroundColor',c_2,'Units','normalized',...
    'position',[0.0 0.1 0.2 0.15],'string','1','visible','off',...
    'Callback',['val = str2num(get(bouton.gfThisPieceContinuosTo,''string''));',...
...
uicontrol('parent',uif,'Style','text','BackgroundColor',c_3,...
    'Units','normalized','position',[0.01 0.05 .15 .9],'string','Tipo de fuente');
% polarisation
para.pol    = 1;
strpol ={'SH','P/SV'};
bouton.pol=uicontrol('parent',uif,'Style','popupmenu',...
    'BackgroundColor',c_2,'Units','normalized',...
    'position',[0.01 0.5 .15 .24],'string',strpol,...
    'Callback','para.pol=get(bouton.pol,''value'');cmd_pol;');

% tipo de fuente
para.fuente=1;
strfuente ={'Onda Plana','Fuente Puntual'};
bouton.fuente=uicontrol('parent',uif,'Style','popupmenu',...
    'BackgroundColor',c_2,'Units','normalized',...
    'position',[0.01 0.25 .15 .24],...
    'string',strfuente,'Callback',...
    'para.fuente=get(bouton.fuente,''value'');para.ninc=1;cmd_fuente;');

% Numero de fuentes
info.ninc     =uicontrol('parent',uif,'Style','text','BackgroundColor',c_3  ,'Units','normalized','position',[0.18 0.75 .07 .24],'string','# fuentes');
para.ninc     =1;
bouton.ninc   =uicontrol('parent',uif,'Style','edit','BackgroundColor',c_2,'Units','normalized','position',[0.18 0.51 .07 .23],...
    'string',para.ninc,'Callback','para.ninc=str2num(get(bouton.ninc,''string''));cmd_ninc;');

% selecion de la onda # i
info.inc    =uicontrol('parent',uif,'Style','text'     ,'BackgroundColor',c_3  ,'Units','normalized','position',[0.18 0.25 .07 .24],'string','Fuente #:');
para.strinc ='1';
bouton.inc  =uicontrol('parent',uif,'Style','popupmenu','BackgroundColor',c_2,'Units','normalized','position',[0.17 0.01 .09 .23],...
    'string',para.strinc,'Callback','cmd_inc;');

% Posicion de la fuente o origen de la fase de las ondas
info.xzs       =uicontrol('parent',uif,'Style','text','BackgroundColor',c_3  ,'Units','normalized','position',[0.26  0.875 .15 .12],'string','Origen fases');
               uicontrol('parent',uif,'Style','text','BackgroundColor',c_3  ,'Units','normalized','position',[0.26  0.75  .15 .12],'string','');
               uicontrol('parent',uif,'Style','text','BackgroundColor',c_3  ,'Units','normalized','position',[0.26  0.75  .05 .12],'string','Xs');
info.ys        =uicontrol('parent',uif,'Style','text','BackgroundColor',c_3  ,'Units','normalized','position',[0.31  0.75  .05 .12],'string','Ys','visible','off');
               uicontrol('parent',uif,'Style','text','BackgroundColor',c_3  ,'Units','normalized','position',[0.36  0.75  .05 .12],'string','Zs');
para.xs(1)= 0;
para.ys(1)= 0;
para.zs(1)= 0;
bouton.xs      =uicontrol('parent',uif,'Style','edit','BackgroundColor',c_2,'Units','normalized','position',[0.26  0.51  .05 .23],...
    'string',para.xs(1),'Callback','para.xs(get(bouton.inc,''value''))=str2num(get(bouton.xs,''string''));dibujo_conf_geo(para,bouton.axe_conf_geo,bouton);');
bouton.ys      =uicontrol('parent',uif,'Style','edit','BackgroundColor',c_2,'Units','normalized','position',[0.31 0.51  .05 .23],'visible','off', ...
    'string',para.ys(1),'Callback','para.ys(get(bouton.inc,''value''))=str2num(get(bouton.ys,''string''));dibujo_conf_geo(para,bouton.axe_conf_geo,bouton);');
bouton.zs      =uicontrol('parent',uif,'Style','edit','BackgroundColor',c_2,'Units','normalized','position',[0.36 0.51  .05 .23],...
    'string',para.zs(1),'Callback','para.zs(get(bouton.inc,''value''))=str2num(get(bouton.zs,''string''));dibujo_conf_geo(para,bouton.axe_conf_geo,bouton);');

info.orient =uicontrol('parent',uif,'Style','text','BackgroundColor',c_3  ,'Units','normalized','position',[0.26 0.375 .15 .12],'string','Orientacion');
% Angulo de las ondas planas incindentes
info.gam    =uicontrol('parent',uif,'Style','text','BackgroundColor',c_3  ,'Units','normalized','position',[0.26 0.25 .07 .12],'string','q','fontname','symbol');
para.gam(1) =0;
bouton.gam  =uicontrol('parent',uif,'Style','edit','BackgroundColor',c_2,'Units','normalized','position',[0.26 0.01 .07 .23],...
    'string',para.gam(1),'Callback','para.gam(get(bouton.inc,''value''))=str2num(get(bouton.gam,''string''));');
% Angulo de las ondas planas incindentes
info.phi    =uicontrol('parent',uif,'Style','text','BackgroundColor',c_3  ,'Units','normalized','position',[0.34 0.25 .07 .12],'string','f','fontname','symbol');%,'visible','off');
para.phi(1) =0;
bouton.phi  =uicontrol('parent',uif,'Style','edit','BackgroundColor',c_2,'Units','normalized','position',[0.34 0.01 .07 .23],...
    'string',para.phi(1),'Callback','para.phi(get(bouton.inc,''value''))=str2num(get(bouton.phi,''string''));','visible','off');

% tipo de onda plana
para.tipo_onda=1;
info.tipo_onda=uicontrol('parent',uif,'Style','text','BackgroundColor',c_3,'Units','normalized','position',[0.42 0.25 .07 .24],'string','Tipo de onda');
strtipo_onda ={'P','SV','R'};
bouton.tipo_onda=uicontrol('parent',uif,'Style','popupmenu','BackgroundColor',c_2,'Units','normalized','position',[0.41 0.01 .09 .24],...
...
info.ptf    = uicontrol('parent',uif,'Style','text'     ,...
    'BackgroundColor',c_3  ,'Units','normalized',...
    'position',[0.705 0.65 .25 .3],'string','Perfil temporal del pulso');
strpulsotps ={'Gaussiana','Ricker (duracion total)','Ricker (tp)','File','Dirac'};
para.pulsotps   = 3;
bouton.pulsotps =uicontrol('parent',uif,'Style','popupmenu',...
    'BackgroundColor',c_2,'Units','normalized','position',[0.705 0.6 .25 .2],...
    'string',strpulsotps,'Callback','para.pulsotps=get(bouton.pulsotps,''value'');');

% duracion del pulso
info.dpf    = uicontrol('parent',uif,'Style','text'     ,...
    'BackgroundColor',c_3  ,'Units','normalized',...
    'position',[0.75 0.4 .09 .2],'string','Duracion del pulso');
para.Ricker_tp   = 2;
bouton.Ricker_tp  =uicontrol('parent',uif,'Style','edit',...
    'BackgroundColor',c_2,'Units','normalized',...
    'position',[0.75 0.2 .09 .2],...
    'string',para.Ricker_tp,'Callback',...
    'para.Ricker_tp=str2num(get(bouton.Ricker_tp,''string''));');

% duracion del retraso
info.dt0    = uicontrol('parent',uif,'Style','text',...
    'BackgroundColor',c_3  ,'Units','normalized',...
    'position',[0.85 0.4 .09 .2],'string','t_0');
para.delais     =-2;
bouton.delais   =uicontrol('parent',uif,'Style','edit',...
    'BackgroundColor',c_2,'Units','normalized',...
    'position',[0.85 0.2 .09 .2],'string',para.delais,...
    'Callback','para.delais=str2num(get(bouton.delais,''string''));');
bouton.pulsoShow = uicontrol('parent',uif,'Style',...
    'pushbutton','Units','normalized', ...
    'position',[0.94 0.2 .06 .1],'string','plot', ...
...
bouton.xri  =uicontrol('parent',uirt0(1),'Style','edit','BackgroundColor',c_2,'Units','normalized','position',[0.25 0.3  .22 .15],...
    'string',para.rec.xri,'Callback','para.rec.xri=str2num(get(bouton.xri,''string''));dibujo_conf_geo(para,bouton.axe_conf_geo,bouton);');
...
bouton.yri  =uicontrol('parent',uirt0(1),'Style','edit','BackgroundColor',c_2,'Units','normalized','position',[0.25 0.15 .22 .15],...
    'string',para.rec.yri,'Callback','para.rec.yri=str2num(get(bouton.yri,''string''));dibujo_conf_geo(para,bouton.axe_conf_geo,bouton);');
...
bouton.zri  =uicontrol('parent',uirt0(1),'Style','edit','BackgroundColor',c_2,'Units','normalized','position',[0.25 0.0  .22 .15],...
    'string',para.rec.zri,'Callback','para.rec.zri=str2num(get(bouton.zri,''string''));dibujo_conf_geo(para,bouton.axe_conf_geo,bouton);');
...
bouton.dxr  =uicontrol('parent',uirt0(1),'Style','edit','BackgroundColor',c_2,'Units','normalized','position',[0.5 0.3  .22 .15],...
    'string',para.rec.dxr,'Callback','para.rec.dxr=str2num(get(bouton.dxr,''string''));dibujo_conf_geo(para,bouton.axe_conf_geo,bouton);');

para.rec.dyr	= 0.2;
bouton.dyr  =uicontrol('parent',uirt0(1),'Style','edit','BackgroundColor',c_2,'Units','normalized','position',[0.5 0.15 .22 .15],...
    'string',para.rec.dyr,'Callback','para.rec.dyr=str2num(get(bouton.dyr,''string''));dibujo_conf_geo(para,bouton.axe_conf_geo,bouton);');

para.rec.dzr	= 0.2;
bouton.dzr  =uicontrol('parent',uirt0(1),'Style','edit','BackgroundColor',c_2,'Units','normalized','position',[0.5 0.0  .22 .15],...
    'string',para.rec.dzr,'Callback','para.rec.dzr=str2num(get(bouton.dzr,''string''));dibujo_conf_geo(para,bouton.axe_conf_geo,bouton);');

% numero de estaciones
para.rec.nrecx= 11;
para.chgnrecx = 0;
bouton.nrecx  =uicontrol('parent',uirt0(1),'Style','edit','BackgroundColor',c_2,'Units','normalized','position',[0.75 0.3  .22 .15],...
    'string',para.rec.nrecx,'Callback','para.rec.nrecx=str2num(get(bouton.nrecx,''string''));para.chgnrecx=1;cmd_recpos;dibujo_conf_geo(para,bouton.axe_conf_geo,bouton);');

para.rec.nrecy= 1;
bouton.nrecy  =uicontrol('parent',uirt0(1),'Style','edit','BackgroundColor',c_2,'Units','normalized','position',[0.75 0.15 .22 .15],...
    'string',para.rec.nrecy,'Callback','para.rec.nrecy=str2num(get(bouton.nrecy,''string''));dibujo_conf_geo(para,bouton.axe_conf_geo,bouton);');

para.rec.nrecz= 1;
bouton.nrecz  =uicontrol('parent',uirt0(1),'Style','edit','BackgroundColor',c_2,'Units','normalized','position',[0.75 0.0  .22 .15],...
    'string',para.rec.nrecz,'Callback','para.rec.nrecz=str2num(get(bouton.nrecz,''string''));dibujo_conf_geo(para,bouton.axe_conf_geo,bouton);');
...
boutonsal.Ut = uicontrol('parent',uipc(1),'Style','checkbox','Units','normalized','position',[0/8 3/4 1/8-eps 1/4-eps],'value',1,'TooltipString','campo total');
