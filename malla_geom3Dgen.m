...
%   subdividir  %
%%%%%%%%%%%%%%%%%
deNuevo = true; maxSteps = 20; iStep = 0;
while deNuevo
  l_faltantes = []; iStep = iStep + 1;
...
%         disp(['lambdaO ',num2str(lambdaObjetivo)])
%         disp(['radioObjetivo ',num2str(radioObjetivo)])
...
%         disp(['at ' num2str(fj) ' on p',num2str(p),' splitting ' num2str(length(l))]);
        if ~isempty(l)
          ...
          % obscutlar índices de los que falte subdividir;
          l = find(para.cont(m,1).piece{p}.subdibData.radios>radioObjetivo);
          l = reshape(l,1,length(l));
          l_faltantes = [l_faltantes , l];
        end
      end
    end
  end
  % revisar si es necesario subdividir de nuevo
  if isempty(l_faltantes); deNuevo = false; end %sum(l_faltantes)==0
...
coord       = struct( ...
  ...
  'CubPtsex',zeros(3,para.gaussian.ngauex,nbpt0)       , ...
...
%     coord.x0(jj) = 
    ...
    % de la inclusión:
    mext	= para.cont1(m-1).vec.mv; %medio exterior
    mint	= para.cont1(m-1).m;  %medio interior
